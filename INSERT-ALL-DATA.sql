-- =====================================================
-- SSTH INVENTORY - INSERT ALL DATA
-- Run this AFTER creating the schema
-- =====================================================

-- =====================================================
-- STEP 1: INSERT YOUR USER PROFILE
-- Get your auth.users ID and create profile
-- =====================================================

-- Insert user profile for the current authenticated user
-- This will use your existing auth.users record
...
SELECT 'User profile created/updated' as status;
...
VALUES
  (uuid_generate_v4(), 'ADM', 'Admin', true, now()),
  (uuid_generate_v4(), 'COAT', 'Coating', true, now()),
  (uuid_generate_v4(), 'MAINT', 'Maintenance', true, now()),
  (uuid_generate_v4(), 'MRKT', 'Marketing', true, now()),
  (uuid_generate_v4(), 'MOLD', 'Mold', true, now()),
  (uuid_generate_v4(), 'PROD', 'Production', true, now()),
  (uuid_generate_v4(), 'PURCH', 'Purchasing', true, now()),
  (uuid_generate_v4(), 'QA', 'QA/QC', true, now()),
  (uuid_generate_v4(), 'RD', 'R&D', true, now()),
  (uuid_generate_v4(), 'SCM', 'SCM', true, now());
...
VALUES
  (uuid_generate_v4(), 'OFFICE', 'Office Supplies', 'Pens, papers, staplers, office items', true, now()),
  (uuid_generate_v4(), 'CLEAN', 'Cleaning Supplies', 'Cleaning products and tools', true, now()),
  (uuid_generate_v4(), 'SAFETY', 'Safety Equipment', 'PPE, safety gear, protective equipment', true, now()),
  (uuid_generate_v4(), 'ELEC', 'Electronics', 'Batteries, computer accessories', true, now()),
  (uuid_generate_v4(), 'UNIFORM', 'Uniforms', 'Work uniforms and clothing', true, now()),
  (uuid_generate_v4(), 'MEDICAL', 'Medical Supplies', 'Masks, gloves, medical items', true, now()),
  (uuid_generate_v4(), 'TOOLS', 'Tools & Equipment', 'Hand tools, maintenance supplies', true, now());

SELECT 'Categories inserted' as status, COUNT(*) as count FROM categories;

-- =====================================================
-- STEP 4: INSERT DEFAULT LOCATION
-- =====================================================

INSERT INTO locations (location_id, location_code, location_name, address, is_active)
VALUES
  (uuid_generate_v4(), 'MAIN', 'Main Warehouse', 'Main Storage Area', true);

SELECT 'Locations inserted' as status, COUNT(*) as count FROM locations;

-- =====================================================
-- STEP 5: INSERT DEFAULT SUPPLIER
-- =====================================================

INSERT INTO suppliers (supplier_id, supplier_code, supplier_name, is_active)
VALUES
  (uuid_generate_v4(), 'DEFAULT', 'Default Supplier', true);
...
-- =====================================================
-- STEP 7: CREATE INVENTORY STATUS FOR ALL ITEMS
-- =====================================================
